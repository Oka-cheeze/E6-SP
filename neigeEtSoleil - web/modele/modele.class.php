...
class Modele {
    ...
    public function insertHabitation($tab, $files) {
        ...
        $requete = "SELECT DATE_DEBUT_RES AS from_, DATE_FIN_RES AS to_ FROM RESERVATION
                    WHERE ID_HABIT = :id AND STATUT_RES = 'confirmee'";
        $prep = $this->pdo->prepare($requete);
        $prep->execute([':id' => $id_habit]);
        foreach ($prep->fetchAll(PDO::FETCH_ASSOC) as $r) {
            $dates[] = ['from' => $r['from_'], 'to' => $r['to_']];
        }

        // 2. Hors saison de location (1er oct -> 14 déc, année courante et précédente)
        $annee = (int)date('Y');
        $dates[] = ['from' => "$annee-10-01", 'to' => "$annee-12-14"];
        $dates[] = ['from' => ($annee-1)."-10-01", 'to' => ($annee-1)."-12-14"];

        // 3. Périodes d'indisponibilité déclarées par le propriétaire
        foreach ($this->getPeriodesDisponibilite($id_habit) as $p) {
            $dates[] = ['from' => $p['DATE_DEBUT'], 'to' => $p['DATE_FIN']];
        }

        return $dates;
    }

    public function insertPeriodeDisponibilite($id_habit, $date_debut, $date_fin) {
        $req = "INSERT INTO PERIODE_DISPONIBILITE (ID_HABIT, DATE_DEBUT, DATE_FIN) VALUES (:id, :debut, :fin)";
        $this->pdo->prepare($req)->execute([':id' => $id_habit, ':debut' => $date_debut, ':fin' => $date_fin]);
    }

    public function getPeriodesDisponibilite($id_habit) {
        $req = "SELECT DATE_DEBUT, DATE_FIN FROM PERIODE_DISPONIBILITE WHERE ID_HABIT = :id";
        $prep = $this->pdo->prepare($req);
        $prep->execute([':id' => $id_habit]);
        return $prep->fetchAll(PDO::FETCH_ASSOC);
    }

    public function deletePeriodesDisponibilite($id_habit) {
        $this->pdo->prepare("DELETE FROM PERIODE_DISPONIBILITE WHERE ID_HABIT = :id")
                  ->execute([':id' => $id_habit]);
    }
    ...
    public function searchHabitations($destination = "", $voyageurs = "") {
        ...
        $sql = "SELECT h.ID_HABIT, h.TITRE_HABIT as TITRE, h.VILLE_HABIT as VILLE, 
                    h.IMAGE_HABIT as IMAGE, h.NB_CHAMBRES_HABIT as NB_CHAMBRES, 
                    h.PRIX_NUIT_HABIT as PRIX_NUIT 
                FROM HABITATION h
                LEFT JOIN STATION s ON h.ID_STATION = s.ID_STATION
...
// 1. POUR L'ADMIN (La nouvelle)
    ...
    public function inscriptionUser($data) {
        if ($data['role'] === 'proprietaire') {
            // Cette requête déclenche le trigger 'trg_insert_proprietaire'
            $requete = "INSERT INTO PROPRIETAIRE (NOM_PRO, PRENOM_PRO, AGE_PRO, ADRESSE_PRO, CP_PRO, VILLE_PRO, TEL_PRO, EMAIL_PRO, MDP_PRO) 
                        VALUES ('" . $data['nom'] . "', '" . $data['prenom'] . "', " . $data['age'] . ", '" . $data['adresse'] . "', '" . $data['cp'] . "', '" . $data['ville'] . "', '" . $data['tel'] . "', '" . $data['email'] . "', '" . $data['mdp'] . "');";
        } else {
            // Cette requête déclenche le trigger 'trg_insert_client'
            $requete = "INSERT INTO CLIENT (NOM_CLI, PRENOM_CLI, AGE_CLI, ADRESSE_CLI, CP_CLI, VILLE_CLI, TEL_CLI, EMAIL_CLI, MDP_CLI) 
                        VALUES ('" . $data['nom'] . "', '" . $data['prenom'] . "', " . $data['age'] . ", '" . $data['adresse'] . "', '" . $data['cp'] . "', '" . $data['ville'] . "', '" . $data['tel'] . "', '" . $data['email'] . "', '" . $data['mdp'] . "');";
        }
        return $this->pdo->exec($requete);
    }
    ...
    public function insertReservation($tab) {
        // CORRECTION BUG 4 : Ajout du PRIX_TOTAL_RES et retour de l'ID
        $requete = "INSERT INTO RESERVATION (ID_USER, ID_HABIT, DATE_RES, DATE_DEBUT_RES, 
        ...
        // On retourne l'ID de la réservation créée
        return $this->pdo->lastInsertId();
    }
